...
class battle_wait_strategy:
    ...
    def __init__(self, *arg, **kwargs):
        self._decorator_options = self._validate_options(kwargs.pop('options', None))
    ...
    def _validate_options(options):
        ...
        for event, value in options.items():
            if not isinstance(event, str) or not isinstance(value, dict):
                raise TypeError(f'options must map event names to dicts, got {options!r}')
        return copy.deepcopy(options)

    def _effective_options(self):
        options = copy.deepcopy(battle_wait_strategy.options or {})
        options.update(copy.deepcopy(self._decorator_options or {}))
    ...
    def with_options(self, options: dict):
        previous_options = self._temp_options
        self._temp_options = self._validate_options(options)
        try:
            with self:
                yield self
        finally:
            self._temp_options = previous_options
