...
截至 2026-08-22，独立的新式神识别器支持 24 个目标，共 109 张带透明蒙版的模板，统一阈值为 `0.85`。
...
2026-08-22 的 `and2` 实机采集中确认 UR 妖刀姬·绯夜猎刃在 `03:58:19..03:58:24` 形成完整轨迹。角色特征为红色短裙、黑色长马尾和裙侧多把银刃，并与官方式神鉴赏核对一致。使用右、中、左 3 张干净帧制作模板，正样本回放分数为 `0.9479 / 0.9207 / 0.9354`；回扫同一时段另外 1808 张截图，最高负样本为 `0.6287`，未达到 `0.85` 阈值。UR 暂时映射为 sidecar 的 SP 兼容类别，以复用稀有目标优先级和双击逻辑。
